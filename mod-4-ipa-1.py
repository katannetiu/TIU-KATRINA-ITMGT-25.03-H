...
def tic_tac_toe(board):
    ...
    capacity = len(board) - 1
    win = len(board)

    horizontal = [sum(x) for x in board]
    vertical = [sum(x) for x in zip(*board)]
    diagonal_up = [sum([board[i][i] for i,v in enumerate(board)])]
    diagonal_down = [sum(board[capacity-i][i] for i,v in enumerate(board))]
    
    if min(horizontal) == -win or min(vertical) == -win or min(diagonal_up) == -win or min(diagonal_down) == -win:
        return ("O")
    elif max(horizontal) == win or max(vertical) == win or max(diagonal_up) == win or max(diagonal_down) == win:
        return ("X")
    else:
        return ("NO WINNER")
